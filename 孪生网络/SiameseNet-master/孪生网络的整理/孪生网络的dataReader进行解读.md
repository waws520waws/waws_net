...
* 主要是我们之前将数据处理完成之后，将数据封装到了pickle文件中，这个部分是将数据从pickle中恢复出来，形成数据集和标签集合
...
* 这个部分是随机的从数据中进行又放回抽样
* 一个batch中，前1/2的数据都从同一个文件夹中抽取，都是相似的图片数据，标签设置为1，后1/2的数据从不同的文件夹中抽取，标签设置为0
* 最后返回一个batch的图片数据和标签数据
    ...
    def get_batch(self, batch_size, mode="train"):
        ...
        for i in range(batch_size):
            ...
            if i >= batch_size // 2:
                ...
                # 如果是i < batch_size // 2，就给他随便加上一个数字，对20求余，使它与category_1不等
...
##### 2.4 创建两张的测试图像（从验证集中随机选取）

* 这个部分主要是随机创建测试数据(我修改了测试方法,这个部分代码弃用)
...
##### 2.5测试孪生网络在 num N个类别一张图片的下的准确性

* 这个部分是对数据进行测试(改变测试方法，弃用)
